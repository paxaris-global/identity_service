...
public class DockerHubService {
    ...
    /**
     * Prepares Docker Hub repository (verifies access, repository will be created on first push)
     * Note: Docker Hub repositories are created automatically when you push an image for the first time.
     * This method verifies authentication and checks if the repository already exists.
     * @param repositoryName The name of the repository (e.g., "username/repo-name")
    ...
    public boolean createRepository(String repositoryName) {
        ...
        try {
            ...
            // Repository doesn't exist yet, but that's OK - it will be created on first push
            log.info("Repository {} will be created automatically on first push", repositoryName);
            return true;
        } catch (Exception e) {
            log.error("Error preparing Docker Hub repository {}: {}", repositoryName, e.getMessage(), e);
            throw new RuntimeException("Failed to prepare Docker Hub repository: " + e.getMessage(), e);
        }
    }
    ...
    public boolean repositoryExists(String repositoryName) {
        try {
            ...
            log.debug("Repository {} does not exist or is not accessible: {}", repositoryName, e.getMessage());
            return false;
        }
    }
     ...
     * Note: Docker Hub v2 API uses Basic Auth, but we can also get a token for some operations
     */
    public String authenticate() {
        try {
            ...
            throw new RuntimeException("Failed to authenticate with Docker Hub: " + e.getMessage(), e);
        }
    }
}
